...
def convert_units(
    ...
    :param x: value to be converted from
    :type x: float | int | np.ndarray | pd.Series
    :param from_: units to convert from
    :type from_: str
    :param to_: units to convert to
    :type to_: str
    :raises ValueError: if the conversion is not found
    :return: value converted to the new units
    :rtype: float | np.ndarray | pd.Series
    """
    from_d = from_.lower()
    to_d = to_.lower()
