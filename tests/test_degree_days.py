...
def test_calc_daily_dbs():
    ...
    daily_dbs = ud.calc_daily_dbs(hpxml)
...
def test_calc_degree_days():
    hpxml = HpxmlDoc(Path("test_hpxmls/ihmh_homes/ihmh4.xml").resolve())
    daily_dbs = ud.calc_daily_dbs(hpxml)
    hdd = ud.calc_degree_days(daily_dbs.f, base_temp_f=65.0, is_heating=True)
    cdd = ud.calc_degree_days(daily_dbs.f, base_temp_f=65.0, is_heating=False)
    assert hdd == 5444.54
    assert cdd == 658.34
...
def test_calc_heat_cool_degree_days():
    hpxml = HpxmlDoc(Path("test_hpxmls/ihmh_homes/ihmh4.xml").resolve())
    daily_dbs = ud.calc_daily_dbs(hpxml)
    degree_days = ud.calc_heat_cool_degree_days(daily_dbs.f)
